...
| **F-3d** | 8× oversampling for the clipper (kill the −34 dB IMD aliasing); targeted, not blanket. | Clipper aliased IMD drops ≥10 dB; no regression. | ◻ spec ready `docs/prompts/SLICE_F3d_oversampler_8x.md`. ⚠️ **attempted & reverted** — Part A (N-stage engine) has a polyphase reconstruction bug for N≠2 to fix first. |
