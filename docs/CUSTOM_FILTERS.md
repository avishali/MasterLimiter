...
| **F-2b** | Wire into `detectCrossover_`/`applyCrossover_`; DEV-tunable spec (audition); phase-coherent reconstruction (use `xDelayed`); detection alignment; fixed-latency Nmax zero-pad; PDC. | Offline-rig Color sweep: low-end dip gone at all values; band sum flat ±0.01 dB; CPU profiled at Color 50. | ✅ **done** — linear-phase crossover + DEV CROSSOVER section; fixed PDC via Nmax padding. |
