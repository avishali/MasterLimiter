...
| **F-2c** | Uniform-partitioned FFT convolution (`PartitionedConvolver`, `juce::dsp::FFT`) under the crossover. ~100× faster than direct. | Matches direct conv <1e-5; complementary sum still exact; latency reported; CPU single-digit % in DAW. | ◻ **NEXT — unblocks F-2b.** Prompt `docs/prompts/SLICE_F2c_partitioned_convolution.md`. ⚠️ refDelay must track convolver's *actual* latency (incl. block latency B), not (N-1)/2. |
| **F-3+** | `HalfbandPolyphaseOS` replaces all 4 `dsp::Oversampling` sites; then movable freq / 3-band / multirate-latency as separate slices. | THD/phase ≥ current baseline (−119 dB passthrough, −81 dB hard-limit). | ◻ |
