...
| **F-3c** | Swap TruePeakDetector(×4) + FinalCeiling oversamplers → `HalfbandPolyphaseOS` (parameterized stage specs; gentle {0.06,90}/{0.12,90} ~62-sample latency = JUCE parity). | No regression; PDC ~unchanged. | ✅ **done** `fd58a4f` — full suite 406 exit 0; **JUCE `Oversampling` now fully gone** (only a comment + unused IspTrimStage remain). No audible/phase impact. |
| **F-3d** | 8× oversampling for the clipper (kill the −34 dB IMD aliasing); targeted, not blanket. | Clipper aliased IMD drops ≥10 dB; no regression. | ✅ **done** SDK `0276c42` (Part A) / plugin `fff7147` (Part B) — **clipper aliased IMD −34 → −45.2 dB (−11.2 dB)**; real cubic IMD −37 dB **unchanged** (inherent cubic, not aliasing); Color still ±0.0001 dB; host latency **2463 identical clipper on/off** (no PDC flip); `mdsp_dsp_tests` exit 0; Release build clean. **Part A** = N-stage `HalfbandPolyphaseOS` (vector `prepare`, fixed the N≠2 polyphase reconstruction; 4× path bit-identical, round-trip ~6e-6). **Part B** = clipper wrapped in a 1-stage 2× instance (4×→8×→clip→4×), OS latency padded to a multiple of 4 for integer PDC, constant path length when clipper bypassed, drive smoother advanced once per 4× sample. **CPU ~22% @ 64-sample buffer / 48 kHz, unchanged clipper on vs off** (the 2× OS round-trip runs unconditionally for constant latency, so the targeted 8× adds no measurable cost — whole-chain budget comfortable at a tight buffer). |
