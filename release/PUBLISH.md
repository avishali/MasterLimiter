...
Version for this beta: **0.3.1-beta**.
...
```bash
cp <produced>.pkg MasterLimiter-0.3.1-beta.pkg
```

## 2. Verify before upload
```bash
shasum -a 256 MasterLimiter-0.3.1-beta.pkg | awk '{print $1}' > MasterLimiter-0.3.1-beta.pkg.sha256
spctl -a -vvv -t install MasterLimiter-0.3.1-beta.pkg     # should say: accepted, notarized
xcrun stapler validate MasterLimiter-0.3.1-beta.pkg        # should validate
# AAX: load in Pro Tools and confirm it authorizes (PACE)
```

## 3. Fill the manifest
Edit `release/beta.json` — replace **`sha256`** (both places) with the 64-hex digest from step 2; bump `build`/`date` if re-releasing. (`download_path`/`release_notes_path` are already correct for 0.3.1-beta.)
...
BUCKET=melechdsp-beta PRODUCT=MasterLimiter PLATFORM=macOS \
  ...
  0.3.1-beta \
  /abs/path/MasterLimiter-0.3.1-beta.pkg \
  /abs/path/MasterLimiter-0.3.1-beta.pkg.sha256 \
  ../MasterLimiter/release/release-notes-0.3.1-beta.md \
  ../MasterLimiter/release/beta.json
```
This uploads (with `--remote`, i.e. the real bucket):
- `MasterLimiter/macOS/0.3.1-beta/MasterLimiter-0.3.1-beta.pkg`
- `…/MasterLimiter-0.3.1-beta.pkg.sha256`
- `…/release-notes.md`
...
- Confirm version/build/date/SHA-256 + release notes show MasterLimiter 0.3.1-beta.
...
---
**Inputs prepared in `release/`:** `release-notes-0.3.1-beta.md`, `beta.json` (fill SHA). The `.pkg` is built locally and is gitignored.
