...
| Auto | **ACTIVE NEXT — Auto-release** | ADR when slice starts | program-dependent release | Program-dependent release with 3 modes. Branch from the new `main`; architect authors its ADR on the HQ branch when it starts. |
...
Note: Slice 7b closed 2026-05-30 and M/S mode is shipped. The active
next slice is Auto-release (program-dependent release, 3 modes), followed
by Slice 16 UI/UX interaction polish, Slice 17 packaging/default-state
audit, then the architect-authored user manual. STFT "Max Transparency"
remains the backlog path to fuller Ozone parity if real-world use
