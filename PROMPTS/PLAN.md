...
| Dev | 🔶 **Implemented — Lookahead ceiling-hold defaults** | — | `dev_lookahead_*` + `ceiling_mode` defaults | Band 2 ms / Wide 5 ms + TruePeak FinalCeiling; fixes Real-mode overs without latency change. Plugin-only. Build/AU + rig done; audition pending. |
| Dev | 🔶 **Implemented — Per-band attack scale experiment** | — | `dev_*_band_attack_scale` + DEV UI + `configureEnvelope` | Slow low / fast high attack-time trim per band; defaults 1× = bit-identical. Plugin-only. Build/AU + rig done; audition pending. |
| Dev | 🔶 **Implemented — Hybrid attack experiment** | `LimiterEnvelope::Hybrid` | `dev_attack_mode` + DEV UI | Pre-ramp + RC follower third mode; default Real unchanged. SDK + plugin build/AU clean 2026-07-04; rig + audition pending. |
| Dev | 🔶 **Implemented — M/S per-band (opt-in)** | — | `dev_band_ms` + band-stage encode/limit/decode | Reuses A2 two-channel path on Mid/Side when M/S mode + toggle on; meter M/S relabel. Default off = bit-identical. Build/AU clean 2026-07-04; audition pending. |
| Dev | 🔶 **Implemented — DEV Band Split visibility** | — | DEV UI layout/labels only | Multiband `band_color` slider moved to visible **Band Split** group; stereo link renamed **Band Stereo**. Build/AU clean 2026-07-04; audition pending. |
| Dev | 🔶 **Implemented — DEV release clarity** | — | `dev_wide_release_scale` + DEV UI relabel/grey | Split High/Wide release trim; clearer DEV labels/tooltips; engine-aware control greying. Build/AU clean 2026-07-04; audition pending. |
...
| Dev | 🔶 **Implemented — 3-band multiband DSP (Slice B)** | ADR-0012 | crossover tree + mid envelope + solo listen | Stage-1 Lo/Mid + stage-2 Mid/Hi linear-phase tree; `envelopeMid_`; N-band Color link; MID GR taps live; per-band solo buttons; DEV hi-split + mid release + Band Link relocated; shipping Color disabled. Build/AU clean; offline null + audition pending. |
| Dev | ✅ **Build/AU — Per-band GR visualization (Slice A)** | — | per-band GR atomics + GR meter + history traces | Metering pass: N=3 atomics and `HistoryFrame` band fields; GR meter LO/MID/HI columns; history band traces. Mid filled by Slice B. |
