...
| Dev | 🔶 **Implemented — Per-band attack scale experiment** | — | `dev_*_band_attack_scale` + DEV UI + `configureEnvelope` | Slow low / fast high attack-time trim per band; defaults 1× = bit-identical. Plugin-only. Build/AU + rig pending audition. |
