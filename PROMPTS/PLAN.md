...
| Dev | 🔶 **Implemented — DEV controls embedded panel (AAX fix)** | — | `DevPanel` overlay in editor | Moved temporary DEV tuning controls from a separate `DocumentWindow` into an in-editor overlay panel toggled by the header **DEV** button (with scrim + close ✕). Fixes AAX/Pro Tools param commit — detached windows are not host-managed. History Graph stays a separate read-only window. UI-only; no DSP/param changes. Build/install/AU validation pending. |
...
editor panel (header **DEV** button), with Character greyed out while `dev_attack_ms` overrides
