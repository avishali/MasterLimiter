...
| 15b | **ACTIVE NEXT — I/O meter features** | — | I/O meters | RMS bar + numeric RMS readout, shared range +/- through existing ScaleMode presets, click-to-reset peak line verification, and one shared centre dB scale between Input and Output meters. |
| 16 | **Beta prep — UI/UX interaction** | — | UI interaction polish | Type-in parameter values / double-click edit, tooltips, label clarity, final layout + resize polish, Color knob placement lock, and product tidy for clip-ballistics file placement if still warranted. |
...
Note: Slice 15 (GR + Clip meter ballistics) closed 2026-05-30. The
active next slice is 15b I/O meter features, followed by Slice 16 UI/UX
interaction polish, Slice 17 packaging/default-state audit, then the
architect-authored user manual. STFT "Max Transparency" remains the
backlog path to fuller Ozone parity if real-world use demands it.
Slice 6 (saturator) and Slice 7b (M/S detection) remain backlogged.
