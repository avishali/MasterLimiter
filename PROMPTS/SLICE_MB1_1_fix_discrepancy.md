...
**BUG 1 — ❌ RETRACTED (phantom, 2026-07-06).** The "setCrossoverFrequencies broken at 16 kHz" was NOT real — it was a stale-file artifact of BUG 0. Clean sweep (fresh `rm` per render): explicit `--crossovers 16000` == default == **range 4.86 / RMS −11.27**, matching all crossovers ≥8 kHz. The two setters AGREE; no crossover bug. Do NOT change the crossover setters.

**BUG 0 — the bench does NOT cleanly overwrite an existing `--out` file (S-C.1's "delete before write" did not take).** Rendering onto a pre-existing output produces a CORRUPT/stale read (confirmed: fresh 16k=−11.27 vs render-over-existing=−14.15). This silently poisoned many ad-hoc file-based bench measurements (mine + likely some of Cursor's). **FIX:** make `writeOutputWav` truncate/recreate the file (open with overwrite; or `std::remove` first). This is prerequisite hygiene — until fixed, only freshly-`rm`'d bench outputs are trustworthy.
